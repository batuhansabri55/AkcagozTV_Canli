...
def link_gercekten_canli_mi(url: str) -> bool:
    """
    ACIMASIZ CANLI YAYIN TESTİ:
    Sadece HTTP status koduna bakmaz, akıştan gerçekten veri gelip gelmediğini doğrular.
    Çalışmayan, boş, sahte yanıt veya 404 veren linkler asla geçemez!
    """
    try:
        with session.get(url, timeout=5, stream=True, allow_redirects=True) as r:
            ...
            if any(t in content_type for t in ['text/html', 'application/json']):
                return False
                
            try:
                # Gerçek veri akıyor mu diye en az 1024 bayt okumayı dene
            ...
            # İçerik içinde sahte hata mesajları var mı kontrol et
            hata_kelimeleri = ["expired", "invalid", "unauthorized", "bad token", "denied", "forbidden", "403", "error", "not found", "<html", "<body"]
            if any(hata in content_text for hata in hata_kelimeleri):
                return False
                
            # Eğer M3U8 listesi döndürdüyse alt akışına (ts/m3u8) da bak ki yayın sahte olmasın
            ...
            # Direct stream veya TS segmenti ise canlı kabul et
            return True
    except Exception: 
        return False
        ...
        if response.status_code == 200 and "#EXTM3U" in response.text:
            ...
            if len(test_edilecekler) >= 3:
                ...
                if calisan_sayisi >= 3:
                    ...
                    # Paneldeki TR kanallarını da son kez canlılık testinden geçir, ölüler elensin
...
def kanal_isleme(kanal_metni: str, kaynak_url: str, eklenen_urller: set):
    ...
    # ACIMASIZ KONTROL: Tüm linkler (özel kaynaklar dahil) canlılık testinden GECMELİDİR!
...
# ==============================================================================
def main():
    logging.info("🛡️ USTA SİSTEM V12.0: ÖLÜ VE ÇALIŞMAYAN LİNKLERE KARŞI ACIMASIZ TEST AKTİF!")
                        ...
                        for s in eski_havuz_satirlari:
                            if s.startswith("#EXTINF"):
                                ...
                                if yasakli_mi(s) or any(yasak_ip in s for yasak_ip in YASAKLI_IP_LISTESI):
                                    ...
                                    continue
                                # Eski havuzdaki linkleri dahi test et, ölmüş olanı listeye alma!
