...
def main():
    ...
    # 2. İNATÇI KANALLAR (Manuel/Saf Linkler)
    # Bu linkler Turkuvaz ve Doğuş grubunun en stabil direkt yayın linkleridir.
    manual_channels = [
    ...
    print("--- Liste Hazırlanıyor ---")

    # Manuel kanalları ekle (ATV buraya dahil)
    ...
    for ch in auto_channels:
        try:
            ...
            if streams and 'best' in streams:
                ...
                print(f"⚠️ {ch['name']} linki alınamadı.")
        except:
            print(f"❌ {ch['name']} hatası.")

    with open("canli.m3u", "w", encoding="utf-8") as f:
        f.write(m3u_content)
    print("\n🚀 İşlem Tamam! TiviMate'i yenile usta.")
