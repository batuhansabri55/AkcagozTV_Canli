...
def main():
    ...
    for s_url in YEDEK_KAYNAKLAR:
        try:
            ...
            count = 0
            # Giniko için en geniş yakalama: Metin içindeki her şeyi dene
            if "giniko" in s_url:
                # onclick, href veya çıplak link fark etmeksizin yakalar
                # (URL, Muhtemel_Isim) ikililerini arar
                g_matches = re.findall(r"(https?://[^\s'\"<>\(\)]+m3u8[^\s'\"<>\(\)]*).*?([A-Z0-9\s\.\-]{3,20})", r.text, re.IGNORECASE)
