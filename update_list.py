...
# SENİN ÖZEL FİLTREN: Sadece bu isimleri içeren kanallar tr.m3u'ya girebilir
KUTSAL_ISIMLER = """
...
# Arama için isimleri hazırla
arama_listesi = [n.strip().lower() for n in KUTSAL_ISIMLER.split(",") if n.strip()]
...
def github_dosya_yaz(icerik, sha):
    ...
    headers = {"Authorization": f"token {GITHUB_TOKEN}", "Accept": "application/vnd.github.v3+json"}
    data = {
        "message": "♻️ 7 Kaynaktan Toplandı ve Süzüldü",
...
def update_m3u():
    ...
    # 1. ADIM: 7 Kaynaktan her şeyi çek
    for s_url in YEDEK_KAYNAKLAR:
        try:
            r = requests.get(s_url, timeout=15)
            r.encoding = 'utf-8'
            matches = re.findall(pattern, r.text)
            ham_havuz.extend(matches)
            print(f"✅ {s_url} kaynağından {len(matches)} kanal çekildi.")
        except:
            continue

    # 2. ADIM: Havuzdan sadece KUTSAL isimleri ayıkla
    for ext_info, ch_name, url in ham_havuz:
        link = url.strip()
        isim = ch_name.strip().lower()
    ...
    print(f"🚀 Bitti: {len(eklenen_linkler)} kanal kaydedildi.")
